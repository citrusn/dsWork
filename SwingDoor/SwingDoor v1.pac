...
package methodNames
	...
	add: 'TagHistory class' -> #fromDb:start:end:;
...
Presenter subclass: #CompressPresenter
	instanceVariableNames: 'startIndex stopIndex stopSpin startSpin trendPosition settingWindow trend miniTrend'
	classVariableNames: ''
	poolDictionaries: ''
	classInstanceVariableNames: ''!
...
StyledContainer subclass: #ChartView
	instanceVariableNames: 'zeroPoint hintBubble dateOrigin selectedPoint scaleY scaleX hbandStep trendColor pointSize penWidth selectedRegion topValue botValue trendRect'
	classVariableNames: ''
	poolDictionaries: ''
	classInstanceVariableNames: ''!
...
fromDb: aTag start: aDateStart end: aDateEnd
	^self new
		tagName: aTag;
		history: (self
					getHistoryTag: aTag
...
!TagHistory class categoriesFor: #fromDb:start:end:!public! !
...
CompressPresenter comment: 'm:=TagHistory sampleMathModel .
CompressPresenter showOn: m'!
...
calcRange
	...
	trendPosition view range: (Interval from: 1 to: r).
	trendPosition model value: 1.
...
createComponents
	...
	startSpin := self add: NumberPresenter new name: 'minSpin'.
	stopSpin := self add: NumberPresenter new name: 'maxSpin'.
	trendPosition := self add: NumberPresenter new name: 'trendPosition'.
...
onViewAvailable
	...
	settingWindow := false.
	s := self model size.
	stopSpin view range: (Interval from: 10 to: s by: 10). 
...
setTrendModel
	| s p b e |
	"self halt."
	s := (stopSpin model value "- startSpin model value + 1") / 2.0.
	p := trendPosition model value - 1.
	b := (s * p) asInteger + 1. "startSpin model value."
	e := (s * p) ceiling + stopSpin model value.
	e := e min: self model size.
	trend model: (self model copyFrom: b to: e).
	miniTrend view selectedRegion: b @ e
...
!CompressPresenter categoriesFor: #onChangedPosition!public! !
...
drawAxisLineFrom: fromPoint to: toPoint on: aGraphics
	| pen |
	pen := GdiplusPen color: self axisColorAsRGB width: self defaultAxisPenWidth.
	aGraphics
...
drawCompressedModelAtGr: graphics
	| pen polygonPoints |
	self selectedRegion ifNotNil: [^ self].
...
drawGridLineFrom: fromPoint to: toPoint on: aGraphics
	| pen |
	pen := GdiplusPen color:  self axisColorAsRGB width: self  defaultAxisPenWidth /2.
	aGraphics
		drawLineFrom: fromPoint
		to: toPoint
		pen: pen
...
drawSelectedRegion: graphics
	"selectedRegion - Point,  где X левая граница, Y правая граница. Индексы в данных модели"
	| x1 x2 y1 y2 |
	self selectedRegion	ifNotNil: 
			[:value |
			"Transcript show: 'selectedRegion: ' , value displayString; cr." "self halt."
			x1 := self scaleTimestampToPxel: (self model at: value x) timestamp.
			x2 := self scaleTimestampToPxel: (self model at: value y) timestamp.
			x2 - x1 < 3 ifTrue: [x2 := x1 + 3	"Transcript show: 'Zero'; cr."].
			y1:= self scaleValueToPixel: topValue.
			y2:= self scaleValueToPixel: botValue.
			
			[graphics compositingQuality: CompositingQualityGammaCorrected.
			"Draw a multicolored background."
			graphics fillRectangle: (x1 @ y1 corner: x2 @ y2 )
				brush: (GdiplusSolidBrush color: (ARGB alpha: 50 red: 255 green: 0 blue: 0)).

			graphics drawRectangle: (x1 @ y1 corner: x2 @ y2)
...
paintStyledEdgesOn: graphics
	...
	(self model isNil or: [self model isEmpty])
		ifFalse: 
			...
			self drawSelectedRegion: graphics.
...
setZeroPoint: anExtent
	...
	trendRect := 0 @ self margintY
				corner: (anExtent x - (1 * self margintY)) @ (anExtent y - self margintY).
	zeroPoint := self margintZeroX @ (trendRect height / 2.0 + (self margintY * 1)) rounded.
	"Transcript
		show: 'anExtent: ' , anExtent displayString , ' trendRect: ' , trendRect displayString
...
showBubbleAt: aPosition  text: aText
	hintBubble	ifNil: 
			...
			hintBubble := (MessageBubble new)
				...
				font: ((Font name: 'Arial' pointSize: 10) characterSet: 204);
...
!ChartView categoriesFor: #axisColorAsRGB!drawing!private! !
...
!ChartView categoriesFor: #defaultMinHBand!geometry!public! !
!ChartView categoriesFor: #defaultNumberHBand!geometry!public! !
!ChartView categoriesFor: #defaultPointSize!geometry!public! !
...
!ChartView categoriesFor: #drawSelectedRegion:!accessing!public! !
...
!ChartView categoriesFor: #scaleDataPointToPixel:!geometry!public! !
!ChartView categoriesFor: #scalePixelToDataPoint:!geometry!public! !
!ChartView categoriesFor: #scalePixelToTimesstamp:!geometry!private! !
!ChartView categoriesFor: #scalePixelToValue:!geometry!private! !
!ChartView categoriesFor: #scaleTimestampToPxel:!geometry!private! !
!ChartView categoriesFor: #scaleValueToPixel:!geometry!private! !
!ChartView categoriesFor: #scaleX!geometry!public! !
!ChartView categoriesFor: #scaleY!geometry!public! !
!ChartView categoriesFor: #selectedRegion!accessing!public! !
!ChartView categoriesFor: #selectedRegion:!accessing!public! !
