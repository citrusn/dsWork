...
value: anObject
	value := anObject! !
!DataPoint categoriesFor: #color!public! !
...
historyAdd: aTimestamp value: aValue
	| point |
	point := (self class dataPointClass) new: aTimestamp value: aValue.
	history add: point.
	^point!
...
dataPointClass
	^DataPointClass ifNil: [DataPointClass := self defaultDataPointClass]!
...
getHistoryTag: aTag start: aDateEnd end: aDateStart
	...
	rs := c query: sql.
	^[rs collect: [:each | DataPoint new: (each atIndex: 2) value: ((each at: 'VALUE') asFloat roundTo: 0.01) ]]
		ensure: [c close]!
...
!TagHistory class categoriesFor: #dataPointClass!public! !
!TagHistory class categoriesFor: #default!private! !
!TagHistory class categoriesFor: #defaultDataPointClass!public! !
